...
def simulate_season(year=2024):
    db = Database()
    print(f"Fetching players and teams for {year} simulation...")
    ...
    profiles = {
        ...
        "WR": {
            ...
            "Depth": {"yds": (15, 45), "rec": (1, 4), "td_prob": 0.1, "chance": 0.85}
        },
        "TE": {
            "Elite": {"yds": (60, 90), "rec": (5, 8), "td_prob": 0.45},
            "Star": {"yds": (35, 60), "rec": (3, 5), "td_prob": 0.25},
            "Starter": {"yds": (15, 35), "rec": (1, 3), "td_prob": 0.15},
            "Depth": {"yds": (10, 30), "rec": (1, 3), "td_prob": 0.05, "chance": 0.85}
        },
        "BACKFIELD": {
    ...
    # Fetch ONLY games for the target year
    games = list(db.db.nfl_games.find({"summary.season.year": year}))
    print(f"Simulating statistics for {len(games)} games in {year}...")
    bulk_updates = []

    for game in games:
        h_id = game['statistics']['home']['id']
        a_id = game['statistics']['away']['id']
        h_team_data = db.db.nfl_teams.find_one({"id": h_id})
        a_team_data = db.db.nfl_teams.find_one({"id": a_id})
        h_full_name = f"{h_team_data['market']} {h_team_data['name']}" if h_team_data else ""
        a_full_name = f"{a_team_data['market']} {a_team_data['name']}" if a_team_data else ""
        ...
        for side, team_name in [("home", h_full_name), ("away", a_full_name)]:
            ...
            for p in skill_receivers:
                ...
                prof = profiles['BACKFIELD'].get(tier, profiles['BACKFIELD']['Depth']) if pos in ['RB', 'FB'] else profiles[pos].get(tier, profiles[pos]['Depth'])
                if tier in ['Depth', 'Role'] and random.random() > prof.get('chance', 1.0): continue
                if pos in ['RB', 'FB']:
                    r_att = random.randint(*prof['rush_att']); r_yds = random.randint(*prof['rush_yds']); r_tds = 1 if random.random() < prof['td_prob'] else 0
                ...
                p_rec = min(rem_comp, random.randint(*prof.get('rec', (1, 2)))); p_yds = min(rem_yds, random.randint(*prof.get('yds', (5, 15)) if 'yds' in prof else prof.get('rec_yds', (5, 10)))) if p_rec > 0 else 0; p_tds = 1 if rem_tds > 0 and random.random() < prof.get('td_prob', 0.1) else 0
    ...
    if bulk_updates:
        ...
        print(f"Successfully simulated statistics for {len(bulk_updates)} games in {year}.")

if __name__ == "__main__":
    year_arg = 2024
    if len(sys.argv) > 1:
        year_arg = int(sys.argv[1])
    simulate_season(year_arg)
